...
impl<'a> Apollo {
    pub fn new(global: Arc<RwLock<Interpreter>>) -> Self {
        Self { global }
    }
    fn exit(code: i32) {
    ...
    fn repl(&self) {
        ...
        loop {
            ...
            
            {
                ...
                if let Some(val) = val {
                    let i = self.global.read().unwrap();
                    println!("({1}) {0}", val.to_string(&self.global.read().unwrap().heap), val.type_as_string(&i.heap));
                }
            }
        }
    }
